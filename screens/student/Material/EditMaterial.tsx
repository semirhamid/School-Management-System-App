...
import CustomMultiSelect from './MultiSelectModal';
...
const EditMaterialModal: React.FC<MaterialProps> = ({ isOpen, onClose, editTitle, id, editDescription, sectionsId, studentsId, documents, operation, screen }) => {
    ...
    const editPostMaterials = async () => {
        ...
        if (screen == "student") {
            ...
            selectedCourse?.forEach((course) => {
                formData.append('Sections', course);
            })
        }
        formData.append('Title', title);
        formData.append('AdderUser', currentUser.name);
        formData.append('Description', description);
        formData.append('UniqueKey', id);
        ...
        try {
            ...
            Alert.alert('Failed', 'Couldnot edit a material');
            if (error.response) {
    ...
    const postMaterials = async () => {
        ...
        if (screen == "student") {
            selectedStudents?.forEach((student) => {
                formData.append('StudentsUsername', student);
            })
        } else {
            selectedCourse?.forEach((course) => {
        ...
        try {
            ...
            Alert.alert('Failed', 'Couldnot add a material');
            if (error.response) {
                // The request was made and the server responded with a status code
                console.log(error.response.data);
                console.log(error.response.status);
    ...
    const deletedUploadedDocument = (documentsList: string[], item: string) => {
        axiosContext?.authAxios
            .patch(LOCAL_BASE_URL + ApiURL.REMOVE_ATTACHEMENT + id + `&filename=${item}`)
            .then((res) => {
                setEditDocuments(documentsList);
            }).catch((err) => {
                console.log(err);
                Alert.alert('Failed', 'Could not remove the attachment material');
            });
    }
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} size="lg">
            <Modal.Content>
                <Modal.CloseButton />
                <Modal.Header>{capitalizeFirstLetter(operation) + " Material"}</Modal.Header>
                <Modal.Body>
                    <VStack space={4}>
                        {
                            screen == "student" ?
                        ...
                        <FormControl>
                            ...
                            <TextArea
                                h={20}
                                placeholder="This is the description of the shared material..."
                                value={description}
